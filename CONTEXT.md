...
- **Chiến lược bóc tách đa tầng**:
  1. *Tầng 1 (Ưu tiên cao nhất)*: Phân tích `preview_json` (payload XHR `/maps/preview/place`) để lấy toàn bộ 27+ trường dữ liệu giàu có.
  2. *Tầng 2*: Bóc tách `APP_INITIALIZATION_STATE` nhúng sẵn trong `html_content`.
  3. *Tầng 3*: Bóc tách DOM HTML thông qua `parse_dom_from_html` (dùng `BeautifulSoup` thuần Python).
  4. *Tầng 4*: Phân tích thành phần địa chỉ chi tiết (`street`, `sublocality`, `district`, `city`, `postal_code`) và khôi phục dấu tiếng Việt chuẩn xác.
